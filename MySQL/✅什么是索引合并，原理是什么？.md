...
索引合并在MySQL 5.1中引入。如以下 SQL 就可能会用到索引合并（分别有两个单独的索引，key1和key2，非联合索引）：
...
在MySQL中，如果进行了索引合并，执行计划中会明确显示type 为index\_merge，key 中会列出用到的索引列表，并且在 extra 中会提示具体用了哪种索引合并策略：
...
* **Using sort\_union：使用排序联合算法，在需要对结果进行排序的查询中，如果不同的排序条件各自有索引，系统可以先分别检索每个索引，然后合并并排序这些结果。**
...
> 更新: 2025-12-28 15:41:07  
> 原文: <https://www.yuque.com/hollis666/vhr2ge/cn34kd6tlw54ulmi>